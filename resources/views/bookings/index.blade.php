...
<body>
    <nav>
        ...
        <div class="nav__btn">
            <a href="{{ route('bookings.create') }}" class="btn"><i class="ri-add-line"></i> Шинэ захиалга</a>
        </div>
    </nav>

    <section class="section__container" style="margin-top: 100px;">
        <h2 class="section__header" style="text-align: center; margin-bottom: 3rem;">ЗАХИАЛГУУД</h2>
            ...
            <table class="table">
                <thead>
                    <tr style="border-bottom: 2px solid var(--primary-color);">
                        ...
                        <th style="padding: 1rem; font-family: var(--header-font); color: var(--text-dark);">Захиалсан огноо</th>
                ...
                <tbody>
                    ...
                    <tr style="border-bottom: 1px solid var(--extra-light); transition: all 0.3s;" onmouseover="this.style.backgroundColor='var(--extra-light)'" onmouseout="this.style.backgroundColor='white'">
                        ...
                        <td style="padding: 1rem; font-weight: 600; color: var(--text-dark);">{{ $booking->customer->name ?? '-' }}</td>
                        <td style="padding: 1rem; color: var(--text-dark);">{{ $booking->car->brand ?? '-' }} {{ $booking->car->model ?? '' }}</td>
                        <td style="padding: 1rem; color: var(--text-light);">{{ optional($booking->booking_date)->format('Y-m-d') ?? '-' }}</td>
                        <td style="padding: 1rem; color: var(--text-light);">{{ optional($booking->start_date)->format('Y-m-d') ?? '-' }}</td>
                        ...
                        <td style="padding: 1rem; text-align: center;">
                            <a href="{{ route('bookings.edit', $booking) }}" class="btn" style="min-width: 80px; margin: 0 5px; font-size: 0.85rem; padding: 0.5rem 1rem; background-color: var(--primary-color);">
                                <i class="ri-edit-line"></i> Засах
                            </a>
                            <form action="{{ route('bookings.destroy', $booking) }}" method="POST" style="display:inline;" onsubmit="return confirm('Та энэ захиалгыг устгахдаа итгэлтэй байна уу?')">
                                @csrf
                                @method('DELETE')
                    ...
                    @empty
                    <tr>
                        <td colspan="8" style="text-align: center; padding: 3rem; color: var(--text-light); font-size: 1.1rem;">Захиалга байхгүй байна</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
