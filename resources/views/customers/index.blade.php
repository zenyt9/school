...
<head>
    ...
    <title>Үйлчлүүлэгчид - Premium Car Rental</title>
</head>
<body>
    <nav>
        ...
        <div class="nav__btn">
            <a href="{{ route('customers.create') }}" class="btn"><i class="ri-add-line"></i> Үйлчлүүлэгч нэмэх</a>
        </div>
    </nav>

    <section class="section__container" style="margin-top: 100px;">
        <h2 class="section__header" style="text-align: center; margin-bottom: 3rem;">ҮЙЛЧЛҮҮЛЭГЧИД</h2>
            ...
            <table class="table">
                <thead>
                    <tr style="border-bottom: 2px solid var(--primary-color);">
                        ...
                        <th style="padding: 1rem; font-family: var(--header-font); color: var(--text-dark);">Хаяг</th>
                ...
                <tbody>
                    ...
                    <tr style="border-bottom: 1px solid var(--extra-light); transition: all 0.3s;" onmouseover="this.style.backgroundColor='var(--extra-light)'" onmouseout="this.style.backgroundColor='white'">
                        ...
                        <td style="padding: 1rem; font-weight: 600; color: var(--text-dark);">{{ $customer->name }}</td>
                        <td style="padding: 1rem; color: var(--text-dark);">{{ $customer->license_number }}</td>
                        <td style="padding: 1rem; color: var(--text-light);">{{ $customer->email ?? '-' }}</td>
                        <td style="padding: 1rem; color: var(--text-dark);">{{ $customer->phone ?? '-' }}</td>
                        <td style="padding: 1rem; color: var(--text-light);">{{ $customer->address ?? '-' }}</td>
                        <td style="padding: 1rem; text-align: center;">
                            <a href="{{ route('customers.edit', $customer->id) }}" class="btn" style="min-width: 80px; margin: 0 5px; font-size: 0.85rem; padding: 0.5rem 1rem; background-color: var(--primary-color);">
                    ...
                    @empty
                    <tr>
                        <td colspan="7" style="text-align: center; padding: 3rem; color: var(--text-light); font-size: 1.1rem;">Үйлчлүүлэгч байхгүй байна.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
